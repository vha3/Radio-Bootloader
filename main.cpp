...
// Collection of all possible chip states
enum CHIPSTATE
{
	BROADCASTING = 0,
	MAYBE_CLIENT,
...
// Collection of all possible message types
enum MESSAGE_TYPE
{
	BROADCAST = 0x01,
	SERVER_REQUEST,
...
static PT_THREAD (protothread_polling(struct pt *pt))
		{
			...
			while(1) {
				...
				if((status & 0x01) && bytes){				//If a sync word has been received & bytes in the FIFO
...
static PT_THREAD (protothread_receiving(struct pt *pt))
		{
			...
			while(1) {
				...
				if (radio.receiveData(&packet) > 0){			    //copy data into global CCPACKET, verify >0 length
...
static PT_THREAD (protothread_sending(struct pt *pt))
		{
			...
			while(1) {
				...
				for(unsigned int i=0; i<packet.length; i++){		//Pack data
